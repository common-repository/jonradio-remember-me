...
define ( 'JR_RM_CAPABILITY', 'manage_options' );

/*	Add Link to the plugin's entry on the Admin "Plugins" Page, for easy access
*/
add_filter( 'plugin_action_links_' . jr_rm_plugin_basename(), 'jr_rm_plugin_action_links' );
...
function jr_rm_plugin_action_links( $links ) {
	/*	Add "Settings" to the end of existing Links
		...
		ref. - https://developer.wordpress.org/reference/hooks/plugin_action_links_plugin_file/
	*/
	array_push(
		$links,
		'<a href="' . admin_url( 'admin.php?page=jr_rm_settings' ) . '">Settings</a>'
		);
	return $links;
}
...
function jr_rm_admin_hook() {
	...
	add_menu_page(
		$jr_rm_plugin_data['Name'], 
		str_ireplace( '.com', '', $jr_rm_plugin_data['Name'] ), 
		JR_RM_CAPABILITY, 
		'jr_rm_settings',
		'jr_rm_settings_page'
	);
...
function jr_rm_settings_page() {
	...
	*/
	if ( ! current_user_can( JR_RM_CAPABILITY ) ) {
		return;
	}
...
?></h1>
	...
	using the 
	<a href="https://wordpress.org/plugins/jonradio-private-site/">My Private Site plugin</a>.
	</p>
	<form action="options.php" method="POST">
	<?php		
	/*	Plugin Settings are displayed and entered here.
	
		First, the security fields for the registered setting "jr_rm_settings"
	*/
	settings_fields( 'jr_rm_settings' );
	/*	Second, display the settings sections and their fields
		(sections are registered for "jr_rm_settings", each field is registered to a specific section)
	*/
...
function jr_rm_admin_init() {
	register_setting(
		...
		'jr_rm_settings',
		array(
			'sanitize_callback' => 'jr_rm_validate_settings'
			)
	);
...
/*	Field Validation upon User Entry.
*/
